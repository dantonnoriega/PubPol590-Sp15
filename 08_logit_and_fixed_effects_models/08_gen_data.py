...
ids = range(1,101) # create 100 id vectors
tz = pytz.timezone('UTC') # this is the timezone smart meters are in
delta = timedelta(minutes=30)
start_datetime = tz.localize(datetime(2015, 1, 1, 0, 0, 0)) # start date of experiment
end_datetime = tz.localize(datetime(2015, 6, 1, 0, 0, 0) - delta) # end date of experiment

np.random.seed(1789)
df = gen_kwh_data(start_datetime, end_datetime, delta, ids)

# export a csv copy without index
df.to_csv(root + "/08_sample_kwh.csv", index = 0)


# GENERATE RANDOM SAMPLE ASSIGNMENTS ------------------------
df1 = DataFrame([(v, np.random.choice([0, 1]),
    np.random.choice(['M','F'], p =[0.3,0.7])) for v in ids],
columns = ['panid', 'assignment', 'gender'])
df1.to_csv(root + "/08_sample_assignments.csv", index = 0)
